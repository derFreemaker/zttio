...
pub const Error = error{WrongImageFormat} || std.Io.Writer.Error;

pub const TRANSMIT_HEADER = "f=32,t=d";

// expects bytes in 'rgba32' format
pub fn transmitOnly(writer: *std.Io.Writer, bytes: []const u8, opts: TransmitOnlyOptions) Error!void {
...
// expects bytes in 'rgba32' format
pub fn transmitAndDisplay(writer: *std.Io.Writer, bytes: []const u8, opts: TransmitAndDisplayOptions) Error!void {
    try writer.print(INTRODUCER ++ "a=T," ++ TRANSMIT_HEADER, .{});
    try writeFlagStruct(writer, opts);
    try writeBytes(writer, bytes);
...
pub fn transmitAnimationFrame(writer: *std.Io.Writer, opts: TransmitAnimationFrameOptions) Error!void {
    ...
    switch (opts.img) {
        .transmit => |transmit| {
            ...
            try writeBytes(writer, transmit.bytes);
        },
        .previous => |prev| {
...
fn writeBytes(writer: *std.Io.Writer, raw_bytes: []const u8) error{WriteFailed}!void {
